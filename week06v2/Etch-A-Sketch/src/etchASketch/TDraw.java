...
public class TDraw {
    ...
    private void superEtchASketch(){
        ...
        String input = in.getText();
        String direction = input.substring(0, 1).toLowerCase();
        String distance = input.substring(1);

        String diagDirect = input.substring(0, 2).toLowerCase();
        String diagDist = input.substring(2);

        if (diagDirect.equals("ne")) {
            t.heading(45);
            distance = diagDist;

        }
        else if (diagDirect.equals("se")) {
            t.heading(135);
            distance = diagDist;

        }
        else if (diagDirect.equals("sw")) {
            t.heading(225);
            distance = diagDist;

        }
        else if (diagDirect.equals("nw")){
            t.heading(315);
            distance = diagDist;
